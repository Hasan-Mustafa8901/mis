...
def _generate_daily_report(backend_data: dict) -> tuple:
    ...
    S = _styles()
    font_bold    = S["font_bold"];    font_head  = S["font_head"]
    font_normal  = S["font_normal"]
    align_center = S["align_center"]; align_left = S["align_left"]
    align_right  = S["align_right"]
    med = S["med"];  thn = S["thn"]
    fill_grey = S["fill_grey"]; fill_blue_hdr = S["fill_blue_hdr"]
    fill_blue_sub = S["fill_blue_sub"]; fill_red = S["fill_red"]
    ...
    def s1_title(row_start):
        ...
        c.font = font_head; c.alignment = align_center; c.fill = fill_grey
    ...
    def s1_section_hdr(row, title):
        ...
        c.font = font_bold; c.alignment = align_center; c.fill = fill_blue_hdr
    ...
    def s1_col_headers(row, d_label="Booking", e_label="Delivery"):
        ...
        for col in range(1, 6):
            ...
            c.font = font_bold; c.alignment = align_center; c.fill = fill_blue_sub
            c.border = Border(
    ...
    def s1_data_row(row, label, b_val, d_val, highlight=False, is_last=False):
        ...
        lc.font = font_normal; lc.alignment = align_left; lc.fill = bg
        lc.border = Border(left=med, right=thn, top=thn, bottom=bottom)
        for col in [2, 3]:
            ws.cell(row=row, column=col).fill = bg
            ws.cell(row=row, column=col).border = Border(left=thn, right=thn, top=thn, bottom=bottom)
    ...
    for i, item in enumerate(_RECON_ROWS):
        ...
        lc, dc, ec = s1_data_row(r, item, booking.get(item, ""), delivery.get(item, ""), hl, is_last)
        if item == "Verification Completion %":
            dc.number_format = "0%"; ec.number_format = "0%"
    ...
    BOOKING_DOC_COLS = ["Customer KYC","Vehicle Details","Price Quotation","Receipts",
                        "Accessories Indent","Exchange Details","MD Reference Approval",
                        "Corp ID","Customer Sign"]
    BOOKING_DOC_KEYS = ["kyc","vehicle","quotation","receipts","accessories_indent",
                        "exchange","md_approval","corp_id","customer_sign"]
    DELIVERY_DOC_COLS = ["Customer Ledger","Tax Invoice","Accessories Indent",
                         "Insurance","RTO","Finance","Evaluation Certificate"]
    DELIVERY_DOC_KEYS = ["ledger","tax_invoice","accessories_indent","insurance",
                         "rto","finance","eval_cert"]
    BASE_COLS = ["S.No.","Date","Customer Name","Mobile No.","TL"]
    BASE_KEYS = ["sno","date","name","mobile","tl"]
    B_COLS = BASE_COLS + BOOKING_DOC_COLS;  B_KEYS = BASE_KEYS + BOOKING_DOC_KEYS
    D_COLS = BASE_COLS + DELIVERY_DOC_COLS; D_KEYS = BASE_KEYS + DELIVERY_DOC_KEYS
    NB = len(B_COLS); ND = len(D_COLS); NBase = len(BASE_COLS)
    ...
    def s2_title(row_start):
        ...
        c = ws2.cell(row=row_start, column=1,
...
def _generate_combine_report(backend_data: dict) -> tuple:
    ...
    def c_col_headers(hdr_row):
        ...
        for di, dealer in enumerate(dealers):
            ...
            c = ws.cell(row=hdr_row, column=b_col,
                        value=dealer.get("name", f"Dealer {di+1}"))
            c.font = font_bold; c.alignment = align_center; c.fill = fill_blue_sub
    ...
    for i, item in enumerate(_RECON_ROWS):
        ...
        if item == "Verification Completion %":
            for dc in dcs: dc.number_format = "0%"
...
if __name__ == "__main__":
    ...
    METRIC = {
        "Total Cases Reported": 100, "Files Received": 95,
        "Files Pending": 7, "Files Incomplete": 2,
        "Files Verified": 86, "Files Approved": 80,
        "Files Rejected": 6, "Verification Completion %": 0.86,
        "Total Discount Given": 450000, "Discount as per Approved Scheme": 400000,
        "Net Excess Discount Amount": 50000, "Highest Discount Car Model": "Swift Dzire",
        "Highest Discount Value": 30000, "Excess Discount Cases": 10,
    ...
    def scaled(n):
        return {k: (round(v * n, 2) if isinstance(v, float) else
                    (v + " " + str(n) if isinstance(v, str) else int(v * n)))
                for k, v in METRIC.items()}

    # ── daily ──────────────────────────────────────────────────────────────
    daily_data = {
        "report_date": "22/05/2026", "scope": "Booking",
        "booking": scaled(1), "delivery": scaled(0.8),
        "booking_files_pending": [
            {"sno": 1, "date": "20/05/2026", "name": "Ramesh Kumar", "mobile": "9876543210", "tl": "Amit S"},
            {"sno": 2, "date": "21/05/2026", "name": "Sunita Devi",  "mobile": "9123456780", "tl": "Priya M"},
        ],
        "delivery_files_pending": [
            {"sno": 1, "date": "19/05/2026", "name": "Anil Sharma", "mobile": "9988776655", "tl": "Rohit K"},
        ],
        "booking_out_of_scope": [], "delivery_out_of_scope": [],
        "booking_delay_files": [], "delivery_delay_files": [],
        "rejected_files_delivered": [],
        "booking_docs_pending": [
            {"sno": 1, "date": "20/05/2026", "name": "Ramesh Kumar", "mobile": "9876543210", "tl": "Amit S",
             "kyc": "Received", "vehicle": "Pending", "quotation": "Received", "receipts": "Partial",
             "accessories_indent": "NA", "exchange": "Pending", "md_approval": "Received",
             "corp_id": "NA", "customer_sign": "Received"},
        ],
        "delivery_docs_pending": [
            {"sno": 1, "date": "19/05/2026", "name": "Anil Sharma", "mobile": "9988776655", "tl": "Rohit K",
             "ledger": "Received", "tax_invoice": "Pending", "accessories_indent": "NA",
             "insurance": "Received", "rto": "Partial", "finance": "Received", "eval_cert": "NA"},
        ],
    }

    # ── combine — 1 dealer ─────────────────────────────────────────────────
    combine_1 = {
        "report_date": "22/05/2026", "scope": "Single Dealer",
        "dealers": [{"name": "Dealer Alpha", "booking": scaled(1), "delivery": scaled(0.8)}],
    }

    # ── combine — 2 dealers ────────────────────────────────────────────────
    combine_2 = {
        "report_date": "22/05/2026", "scope": "All Dealers",
        "dealers": [

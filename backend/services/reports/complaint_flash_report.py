...
from backend.services.report.base_report import BaseReport
import time
...
class ComplaintFlashReport(BaseReport):
    ...
    def ensure_one_pager(self):
        if self.get_y() > self.MAX_Y:
            raise ValueError("Flash Report exceeds. Please shorten the content.")
    ...
    def build(self, data: dict):
        ...
        self.key_value_row(
            "Designation of Complainant", data["designation_complainant"]
        )
        ...
        self.key_value_row(
            "Name of Audit Assistant (AA) stationed at Complainant",
            data["complainant_aa"],
        )

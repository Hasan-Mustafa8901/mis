...
def get_user():
    return {
        ...
        "allowed_outlet_ids": app.storage.user.get("allowed_outlet_ids"),
    }
...
def render_topbar(page_label: str) -> None:
    ...
    with ui.header().classes(
        ...
        with (
            ui.row()
            .classes(
                "items-center gap-3 hover:bg-white/5 px-3 py-1.5 rounded-lg transition cursor-pointer"
            )
            .on("click", lambda: user_menu.open())
        ):
            ...
            ui.icon("expand_more").classes("text-white/45 text-[18px]")

            with (
                ui.menu()
                .props('anchor="bottom right" self="top right"')
                .classes(
                    "w-[230px] rounded-xl shadow-2xl border border-gray-100 overflow-hidden bg-white p-0"
                ) as user_menu
            ):
                with ui.column().classes("gap-0 w-full p-0"):
                    with ui.row().classes(
                        "w-full items-center gap-3 px-4 py-3 bg-[#0F1623] border-b border-[#E8402A]/50"
                    ):
                        with ui.element("div").classes(
                            "w-9 h-9 rounded-full bg-[#E8402A] flex items-center justify-center text-white font-bold text-sm shadow"
                        ):
                            ui.label(initials if len(initials) <= 3 else initials[:2])
                        with ui.column().classes("gap-0 min-w-0"):
                            ui.label(name).classes(
                                "text-[13px] text-white font-semibold leading-tight truncate"
                            )
                            ui.label(role_d.title()).classes(
                                "text-[10px] text-white/55 tracking-wide leading-tight truncate"
                            )

                    with ui.menu_item(on_click=handle_logout).classes(
                        "w-full px-4 py-2.5 text-red-600 hover:bg-red-50 transition-colors"
                    ):
                        with ui.row().classes("items-center gap-3 w-full"):
                            ui.icon("logout").classes("text-[18px]")
                            ui.label("Logout").classes("text-[13px] font-semibold")
...
async def dashboard_page() -> None:
    ...
    async def load_dashboard_data():
        ...
        txns: list[dict] = await api_get("/dashboard-data", params=params) or []
    ...
    with ui.row().classes("w-full no-wrap items-stretch min-h-[calc(100vh-52px)]"):
        ...
        async def refresh_export_jobs():
            try:
                ...
                logger.exception("Error refreshing export jobs: %s", str(e))
...
def build_complaint_action_bar(state: FormState) -> None:
    ...
    with ui.row().classes("w-full items-center justify-between py-4"):
        ...
        redirect_page_path = (
            "/complaints-ctrl" if user_role in ["admin"] else "complaints-dashboard"
        )
        ui.button(
            "← Back to Complaints", on_click=lambda: ui.navigate.to(redirect_page_path)
        ).classes("text-gray-500 text-[13px] hover:text-gray-800").props("flat no-caps")
...
async def complaints_dashboard_page():
    ...
    for complaint in all_complaints:
        ...
        if complaint.get("complainee_outlet_id") in current_user.get(
            "allowed_outlet_ids"
        ):
            complaints_against.append(complaint)

        # Complaint RAISED BY current user (they are the complainant)
        if complaint.get("complainant_outlet_id") in current_user.get(
            "allowed_outlet_ids"
        ):
                        ...
                        if not selected_complaint["data"]:
                            with remarks_display:
                                with ui.card().classes("w-full p-8 text-center"):
                                    ui.icon("info", size="xl").classes(
                                        "text-gray-300 mb-2"
                                    )
                                    ui.label(
                                        "Select a complaint to view and add remarks"
                        ...
                        with remarks_display:
                            ...
                            with ui.card().classes("w-full p-5 mb-4"):
                                ...
                                with ui.row().classes("w-full mb-3 pb-3 border-b"):
                                    with ui.column().classes("w-full gap-1"):
                                        ...
                                        ui.label(
                                            complaint.get("remarks_complainant")
                                            or "No remarks"
                                ...
                                with ui.row().classes("w-full mb-3 pb-3 border-b"):
                                    with ui.column().classes("w-full gap-1"):
                                        ...
                                        ui.label(
                                            complaint.get("remark_complainee_aa")
                                            or "No remarks"
                                        ).classes("text-sm text-gray-700")
                            ...
                            with ui.card().classes("w-full p-5"):
                                ...
                                async def submit_remark():
                                    if (
                                        not remark_input.value
                                        or not remark_input.value.strip()
                                    ):
                                        ui.notify(
                                            "Please enter a remark", type="warning"
                                        )
                                        return
                                    ...
                                    try:
                                        ...
                                        ui.notify(
                                            "Remark added successfully", type="positive"
                                        )
                                        ...
                                        try:
                                            ...
                                            all_complaints_updated = response.get(
                                                "rows", []
                                            )
                                ...
                                with ui.row().classes("w-full justify-end gap-2 mt-3"):
                                    ui.button(
                                        "Clear",
                                        on_click=lambda: remark_input.set_value(""),
                                    ).props("outline color=grey")
                                    ui.button(
                                        "Submit Remark", on_click=submit_remark
                                    ).props("color=primary").classes("bg-[#E8402A]")
